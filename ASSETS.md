...
**개수**: 세계 14개국 + 한국 시/군/구 17개 + 한국 시/도 17개 = 48종
...
### 나라·지역 48종 → `assets/nations/{id}.png`
...
`cheongyang`(청양) · `icheon`(이천) · `cheongju`(청주)
...
`jeonbuk`(전북특별자치도) · `jeonnam`(전라남도) · `gyeongbuk`(경상북도) · `gyeongnam`(경상남도) · `jeju_do`(제주특별자치도)

> ⚠️ 시/군/구의 `busan`과 시/도의 `busan`은 **다른 폴더 규칙 없이 같은 파일명**입니다.
> 두 그림이 다르게 필요하다면(시/군/구는 좁은 지역, 시/도는 광역시) 파일 하나를 만드신 뒤
> 나머지는 저에게 말씀해 주시면 구분되게 코드를 조정해 드릴게요.

> 💡 **한국 지역 문장은 이렇게 만들면 좋습니다**: 그 지역 특산물 + 그 지역 상징물.
> 예를 들어 제주는 감귤과 한라산, 부산은 물고기와 바다·다리, 평창은 감자와 눈 덮인 산,
> 완도는 전복과 파도, 청주는 상당산성과 포도, 시/도 단위인 경기도는 넓은 들판과 포도.
